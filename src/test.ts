...
async function test() {
  ...
  const up: UserProfile = await l.user(process.env['LINKEDIN_PROFILE_ID'] as string);
  ...
  await Promise.all(
    [l.user(process.env['LINKEDIN_PROFILE_ID'] as string, true).then(async (u) => {
      const xps = await u.experiences();
      console.info(`User experiences: ${JSON.stringify(xps, undefined, 4)}`);
      await u.close();
    }),
    l.user(process.env['LINKEDIN_PROFILE_ID'] as string, true).then(async (u) => {
      const es = await u.education();
